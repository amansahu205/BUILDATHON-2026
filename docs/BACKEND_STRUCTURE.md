...
> Runtime: Node.js 22.13.0 LTS | Framework: Fastify 5.2.1 | ORM: Prisma 6.3.1
...
VERDICT's backend is a **Fastify monolith with clear module boundaries**, exposing:
...
> **Convention:** All tables use snake_case. All IDs are CUID (not UUID) generated by Prisma. All timestamps are UTC. All monetary values and scores are stored as integers or floats, never VARCHAR.
...
| `weakness_map_scores` | `JSONB` | NULL | `{ timeline: 78, financial: 34, communications: 65, relationships: 80, actions: 71, prior_statements: 55, composure: 60 }` |
...
```typescript
// Prehandler on all /api/v1 routes except public
...
// Resource ownership check (call after requireAuth)
function requireFirmOwnership(resourceFirmId: string, userFirmId: string) {
  if (resourceFirmId !== userFirmId) throw new ForbiddenException('FORBIDDEN');
}
```
...
```
Algorithm:   bcrypt (node.bcryptjs 5.1.1)
...
### Zod Schemas (shared between frontend React Hook Form and backend Fastify)
...
3. **SQL injection** — Prisma parameterized queries (no raw SQL with interpolation)
...
fastify.setErrorHandler((error, request, reply) => {
  ...
  // Log all errors to Pino + Sentry
  fastify.log.error({ error, requestId }, 'Request error');
  ...
  // Known app errors (thrown explicitly)
...
fastify.post('/auth/login', {
  config: { rateLimit: { max: 10, timeWindow: '1 minute', keyGenerator: r => r.body.email } }
}, loginHandler);
```
...
```
{YYYYMMDD}_{HHMMSS}_{verb}_{entity}_{field}
Examples:
  20260221_140000_create_initial_schema
  20260222_090000_add_behavioral_sentinel_columns
  20260225_110000_add_session_events_emotion_vectors
  20260301_150000_drop_legacy_alert_status_field (DESTRUCTIVE — two-phase)
```
...
npx prisma migrate dev --name "add_behavioral_sentinel_columns"
# → Generates SQL diff + applies to dev DB

# Review the generated SQL before committing:
cat prisma/migrations/20260222_090000_add_behavioral_sentinel_columns/migration.sql

# Staging — apply without interactive prompts
npx prisma migrate deploy

# Production — same command; runs in CI before server restart
npx prisma migrate deploy
...
1. Engineer creates migration on feature branch:
   ...
   → migration.sql generated in prisma/migrations/

2. PR opened → CI runs:
   - npx prisma validate (schema syntax check)
   - npx prisma migrate deploy on test DB
...
4. Merge to main → production deploy sequence:
   a. npx prisma migrate deploy (runs BEFORE server restart)
   b. New Fastify server starts (new code works with migrated DB)
...
```bash
# Prisma does NOT auto-rollback data migrations.
...
1. Mark migration as rolled back (metadata only — does not undo SQL):
   npx prisma migrate resolve --rolled-back "20260222_090000_add_x"

2. Manually write + run reverse SQL:
   ALTER TABLE sessions DROP COLUMN IF EXISTS behavioral_sentinel;
...
```
Development:  Prisma built-in pool (connection_limit=5 in DATABASE_URL)
Production:   PgBouncer in transaction mode
              Fastify connects to PgBouncer (port 6432), not Postgres directly
              Pool size: 20 connections per Fastify instance
              Max PostgreSQL connections: 100
              PgBouncer config: pool_mode=transaction, max_client_conn=200

DATABASE_URL (production, via PgBouncer):
  postgresql://verdict:pass@pgbouncer:6432/verdict_prod?connection_limit=20&pool_timeout=30
```
...
# 4. Run prisma migrate deploy (verify schema is current)
# 5. Resume traffic
...
> **Current actual structure** (`verdict-backend/src/` — as built):

```
verdict-backend/
├── prisma/
│   ├── schema.prisma           # Full 11-model schema (source of truth)
│   └── seed.ts                 # Dev seed: firm, attorney, demo case
├── src/
│   ├── index.ts                # Fastify bootstrap: CORS, rate-limit,
│   │                           #   error handler, route registration,
│   │                           #   /api/v1/health endpoint
│   ├── agents/
│   │   ├── detector.ts         # Inconsistency Detector Agent
│   │   │                       #   (Nia search + Nemotron scoring)
│   │   ├── interrogator.ts     # Interrogator Agent
│   │   │                       #   (claudeStream + Nia context)
│   │   └── objection.ts        # Objection Copilot Agent
│   │                           #   (FRE classification via claudeChat)
│   ├── lib/
│   │   └── db.ts               # Prisma client singleton
│   ├── middleware/
│   │   ├── auth.ts             # requireAuth preHandler
│   │   │                       #   (JWT verify → attach request.user)
│   │   ├── error.ts            # Global Fastify error handler
│   │   │                       #   (ZodError + VerdictError formatting)
│   │   └── rate-limit.ts       # Rate limit config constants
│   ├── routes/
│   │   ├── auth.ts             # POST /login, POST /logout
│   │   ├── briefs.ts           # GET  /briefs/:briefId
│   │   ├── cases.ts            # GET|POST /cases, GET|PATCH|DELETE /cases/:id
│   │   └── sessions.ts         # GET|POST /sessions/:id/start
│   │                           #   + /agents/question (SSE stream)
│   │                           #   + /agents/objection
│   │                           #   + /agents/inconsistency
│   └── services/
│       ├── claude.ts           # claudeChat() + claudeStream()
│       ├── elevenlabs.ts       # textToSpeech() + speechToText()
│       ├── nemotron.ts         # scoreContradiction()
│       └── nia.ts              # indexDocument() + searchIndex()
├── package.json
├── tsconfig.json               # noEmit typecheck config
├── tsconfig.build.json         # outDir: dist, for production build
└── .env.example                # All required env var keys
```

> **Planned additions** (add as features are implemented):

```
src/
├── lib/
│   ├── redis.ts                # ioredis singleton
│   ├── s3.ts                   # AWS S3 presign + multipart upload
│   ├── email.ts                # Resend transactional email
│   └── errors.ts               # VerdictError class hierarchy
├── routes/
│   ├── documents.ts            # presign, confirm-upload, ingestion-status
│   ├── witnesses.ts            # CRUD witnesses
│   ├── alerts.ts               # PATCH alert decisions
│   ├── admin.ts                # Admin panel routes
│   └── users.ts                # GET/PATCH /users/me
├── agents/
│   ├── behavioral.ts           # Behavioral Sentinel (FACS → alert)
│   └── orchestrator.ts         # Review Orchestrator (brief synthesis)
└── types/
